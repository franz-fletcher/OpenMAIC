#!/usr/bin/env node
/**
 * Downloads the Qwen Audio 3.0 TTS base-voice workbooks published by Alibaba
 * Cloud and emits a deterministic, prettier-clean TypeScript data module at
 * lib/audio/data/qwen-token-plan-base-voices.ts.
 *
 ...
 *
 * Pass --verify to regenerate to a temp file and compare semantic records plus
...
const SOURCES = [
  ...
  {
    url: 'https://help-static-aliyun-doc.aliyuncs.com/file-manage-files/en-US/20260723/fbrqbk/qwen-audio-3.0-tts-flash-base-voices-en.xlsx',
...
function formatModule(records) {
  ...
  lines.push(' *');
  lines.push(' * Sourced from the Alibaba Cloud Qwen Audio 3.0 TTS base-voice workbooks.');
  lines.push(' * To regenerate, run:');
  lines.push(' *   node scripts/generate-qwen-token-plan-voices.mjs');
  lines.push(' *');
  lines.push(` * ${records.length} records (plus + flash), sorted by model then id.`);
  lines.push(' */');
  lines.push('');
