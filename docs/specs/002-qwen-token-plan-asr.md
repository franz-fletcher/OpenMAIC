...
5. As an operator, I want clear error text for rejected audio, so that the settings test shows an actionable message.
...
**Store default.** Add the provider to `asrProvidersConfig` defaults at `lib/store/settings.ts:521-528`. Set `apiKey: ''`, `baseUrl: ''`, `modelId: 'qwen-audio-3.0-asr-flash'`, `enabled: false`. Mirror the `qwen-asr` default. The `ensureBuiltInAudioProviders` loop at lines 718-724 injects the default automatically for new registry ids.
...
The live test at `tests/audio/qwen-token-plan-asr-live.test.ts` is opt-in. It skips unless `TEST_LOAD_LOCAL_ENV` is `1` and `ASR_QWEN_TOKEN_PLAN_API_KEY` is present. The skip message mirrors batch 001. The audio source is the vendor's public sample `hello_world_female2.wav`, fetched at test time and sent as a base64 data URI (operator decision Q1-a). When enabled, it transcribes that sample, prints `[qwen-token-plan-asr-live] ok`, and asserts non-empty text. The adversarial gate runs it with the env loader. The skip gate runs it without.
