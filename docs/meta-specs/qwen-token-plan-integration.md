...
- Response text sits at top-level `.text`. It is not under `output.choices`. The repo parser at `lib/audio/asr-providers.ts:418-420` reads `output.choices`, so it throws on this shape.
