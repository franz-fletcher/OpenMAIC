...
# Qwen Token Plan TTS
TTS_QWEN_TOKEN_PLAN_API_KEY=
TTS_QWEN_TOKEN_PLAN_BASE_URL=     # optional override

# Any OpenAI-compatible TTS endpoint

Supported TTS prefixes include `TTS_OPENAI_`, `TTS_AZURE_`, `TTS_GLM_`, `TTS_QWEN_`, `TTS_QWEN_TOKEN_PLAN_`, `TTS_MINIMAX_`, `TTS_DOUBAO_`, `TTS_ELEVENLABS_`, `TTS_VOXCPM_`, and `TTS_LEMONADE_`.
...
# Qwen Token Plan ASR
ASR_QWEN_TOKEN_PLAN_API_KEY=
ASR_QWEN_TOKEN_PLAN_BASE_URL=     # optional override
...
Supported provider prefixes for ASR include `ASR_OPENAI_`, `ASR_QWEN_`, `ASR_QWEN_TOKEN_PLAN_`, `ASR_AZURE_`, `ASR_FUNASR_`, and `ASR_LEMONADE_`.
...
### LLM and image/video token-plan providers

The `QWEN_*`, `IMAGE_QWEN_*`, and `VIDEO_HAPPYHORSE_*` prefixes cover the LLM, image, and video modalities of the token plan. See the [Token Plan](#token-plan) section or use the Settings preset for a one-key setup.
...
### Token-plan YAML example

You can configure token-plan providers in YAML. See [`server-providers.yml.example`](https://github.com/THU-MAIC/OpenMAIC/blob/main/server-providers.yml.example) for a complete sample.
